...
export class GoogleTranslateCommand implements Command {
    public metadata: MessageApplicationCommandData = {
        name: Lang.getCom('commands.gtl'),
    ...
    public requirePerms = [];
    public gtlEmoji = '<:gtl:1058828948437471252>';
...
/*
    ...
    public keyword(langCode: LangCode): string {
        return Lang.getRef('commands.gtl', langCode);
    }

    public regex(langCode: LangCode): RegExp {
        return Lang.getRegex('commandRegexes.gtl', langCode);
    }
    ...
    public async executeMessage(msg: Message, args: string[], data: EventData): Promise<void> {
        ...
        if (msg.type == "REPLY") {
            ...
            const emoji = msg.client.emojis.cache.find(e => e.name === 'gtl');
            let tl = await ApiUtils.GoogleTranslate(text);
            if (tl != undefined) {
                tl = `${emoji}:${tl}`;
            }
        ...
        if (url == undefined && args.length === 2) {
            await MessageUtils.send(
                msg.channel,
                Lang.getEmbed('displayEmbeds.gtlHelp', data.lang())
            );
            return;
        }
        if (url !== undefined) {
